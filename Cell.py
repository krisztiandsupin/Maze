...
class Cell:
    ...
    def color(self, screen, color, graph_bool = False, coordinate_text_bool = False):
        pygame.draw.polygon(screen, color, self.border_points)
        l = len(self.walls)

        for i in range(0, l):
            if self.walls_bool[i] == True:
                pygame.draw.line(screen, MazeColor.line, self.walls[i][0], self.walls[i][1])
            else:
                pygame.draw.line(screen, color, self.walls[i][0], self.walls[i][1])

        for point in self.border_points:
            pygame.draw.line(screen, MazeColor.line, point, point)
    ...
    def color_grid(self, screen, color, graph_bool = False, coordinate_text_bool = False):
        pygame.draw.polygon(screen, color, self.border_points)
        l = len(self.walls)
