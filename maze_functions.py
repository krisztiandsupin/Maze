...
def rectSize(screen,n, marginsize, labelsize):    #calculate the optimal rectangle size
    a = math.floor((min(screen[0], screen[1]) / 2 - marginsize-labelsize)/n)
    return a
...
def drawRect(screen, x, y, a, T, bgrcol, lncol, line_thickness = 1):
    temp=(x,y)
...
def drawGrid_rectengular(screen,x,y,n,m,a,Grid,mazecol):
    bgrcol=mazecol[0]
    lncol=mazecol[1]
    for i in range(0,n):
        for j in range(0,m):
            drawRect(screen, x + j*a, y + i*a, a, Grid[i*m+j], bgrcol, lncol)

    if mazecol[2] != mazecol[0] and mazecol[3] != mazecol[0]:
        colorRegion_rectengular(screen,x,y,n,m,a,Grid,0,mazecol[2])
        colorRegion_rectengular(screen,x,y,n,m,a,Grid,n*m-1,mazecol[3])
...
def edgeGen_circle(n, vertex_list):
    edge_list = []
    list_length = len(vertex_list)

    for i in range(1, list_length):
        #in a circle
        temp_list_length = len(vertex_list[i])
        for j in range(0, temp_list_length):
            edge_list.append(vertex_list[i][j], vertex_list[i][j] + 1)
        edge_list.append((0, vertex_list[0][temp_list_length]))

    for i in range(0, list_length - 1):
        temp_list1_length = len(vertex_list[i])
        temp_list2_length = len(vertex_list[i+1])
        for j in range(0, temp_list1_length):
            level_ratio = temp_list2_length // temp_list1_length
            for k in range(0, level_ratio):
                edge_list.append((vertex_list[i][j], vertex_list[i + 1][j * level_ratio + k]))
...
def regionGen_rectengular(n, m):
    Regions = [[i] for i in range(0, n*m)]
    return Regions
...
def gridGen_rectengular(n, m): # n x m rect list with bools for edges
    Grid=[[True,True,True,True] for i in range(0,n*m)]
    return Grid
...
def mazePic(n,Grid,mazecol,flname):
    ...
    pygame.image.save(win, 'testmazes\size{0}\{1}'.format(n,filename))
    print("file {} has been saved".format(filename))
    # update the display window to show the drawing

def mazePic_rectengular(n, m, a, Grid, mazecol, flname):
    win = pygame.Surface((2*m*a + 200, 2*n*a + 200))
    print(a)
    win.fill(colors.white)
    x = 100
    y = 100
    drawGrid_rectengular(win, x, y, n, m, 2 * a, Grid, mazecol)
    filename = "{0}.png".format(flname)
...
def mazeLogfc(a,b):
    for i in range(a, b, 5):
        ...
        sheet['B3'] = 'algorithm'
        sheet['C3'] = 'date&time'
        book.save('testmazes\size{0}\logfile_maze{1}.xlsx'.format(i, i))

def mazeLog(n,id,algorithm):
    book = openpyxl.load_workbook('testmazes\size{0}\logfile_maze{1}.xlsx'.format(n, n))
    sheet = book.active
    time = datetime.datetime.now().strftime('%Y:%m:%d %H:%M:%S')

    sheet.cell(id + 4, 1).value = "maze{0}_{1}".format(n,id)
    sheet.cell(id + 4, 2).value =algorithm
    sheet.cell(id + 4, 3).value = time

    id += 1
    sheet.cell(2, 2).value = id
    book.save('testmazes\size{0}\logfile_maze{1}.xlsx'.format(n, n))
...
def edgeErase_rectengular(x,y,n,m,a,line_thickness,k,l): #n=maze dimension, (x,y)=coordinates of griid upper-left point, k,l=grid k th, l th region (k<l)
    if k < l:
        t = l
        l = k
        k = t
    if abs(k-l) == 1:
        begincdList=[x + (k % m)*a, y + (k // m)*a + line_thickness] #begin coordinates
        endcdList=[x + (k % m)*a, y + ((k // m)+1)*a - line_thickness] #end coordinates
    else:
        begincdList = [x + (k % m) * a + line_thickness, y + (k // m) * a]  # begin coordinates
        endcdList = [x + (k % m+1) * a - line_thickness, y + (k // m) * a]  # end coordinates
    return begincdList,endcdList
...
def gridEdgeremove_rectengular(Grid,n,m,edge0,edge1):
    if edge1-edge0 == 1:
        Grid[edge0][1]=False
        Grid[edge1][3]=False
    elif (edge1-edge0) == m:
...
def colorRegion_rectengular(screen, x, y, n, m, a, Grid, region, col):
    pygame.draw.rect(screen, col, [x + (region % m) * a, y + (region // m) * a, a + 1, a + 1], 0)
    drawRect(screen, x + (region % m) * a, y + (region // m) * a, a, Grid[region], col, lncol)
...
def mazeUpdate_rectengular(n,m,Edges,Maze,Grid,edge0,edge1):
    remEdge = (edge0, edge1)
    Edges.remove(remEdge)
    Maze.append(remEdge)
    Grid = gridEdgeremove_rectengular(Grid, n, m, edge0, edge1)
    return Edges,Maze,Grid
...
def mazeCreate_rectengular(n, m):
    Edges = edgeGen_rectengular(n, m)
    Grid = gridGen_rectengular(n, m)
    Maze = []
    return Edges,Maze,Grid
...
def colorHexregion(screen, x, y, n, a, Grid, Hexagonal, region, col):
    ...
    drawHex(screen,x-m*(a*math.cos(math.pi/6))+column*(2*a*math.cos(math.pi/6)),y+row*(a+a*math.sin(math.pi/6)),a,Grid[region],col,colors.black)
...
def edgeHexerase(screen,x, y, n, a,Hexagonal ,line_thickness, edge0, edge1): #n=maze dimension, (x,y)=coordinates of griid upper-left point, k,l=grid k th, l th region (k<l)
    ...
    if row0==row1: #in the same row
        pygame.draw.line(screen, colors.white, (xcord0 + 2 * a * math.cos(math.pi / 6), ycord0),
                         (xcord0 + 2 * a * math.cos(math.pi / 6), ycord0 + a), 1)
    elif row1<=n-1 and column0==column1: #same upper right diagonal
        pygame.draw.line(screen, colors.white, (xcord0 + a * math.cos(math.pi / 6), ycord0 + a + a * math.sin(math.pi / 6)), (xcord0, ycord0 + a),
                         1)
    elif row1>=n and column0==column1+1: #same lower right diagonal
        pygame.draw.line(screen, colors.white, (xcord0 + a * math.cos(math.pi / 6), ycord0 + a + a * math.sin(math.pi / 6)), (xcord0, ycord0 + a),
                         1)
    elif row1<=n-1 and column0==column1+1: #same upper left diagonal
        pygame.draw.line(screen, colors.white, (x + 2 * a * math.cos(math.pi / 6), y + a),
                         (x + a * math.cos(math.pi / 6), y + a + a * math.sin(math.pi / 6)), 1)
...
def mazeGenFolders(a,b):
    for n in range(a,b+1):
        if os.path.isdir("testmazes\size{0}".format(n)) == False:
            dir="testmazes\size{0}\logfile_maze{1}.xlsx".format(n,n)
            os.makedirs("testmazes\size{0}".format(n))
            wb = openpyxl.Workbook()
            wb.save(dir)

            book = openpyxl.load_workbook(dir)
            sheet = book.active
            sheet.cell(1, 1).value ='size'
            sheet.cell(1, 2).value =n
            sheet.cell(2, 1).value ='generated mazes'
            sheet.cell(2, 2).value =0
            sheet.cell(3, 1).value ='filename'
            sheet.cell(3, 2).value ='algorithm'
            sheet.cell(3, 3).value ='date&time'
            book.save("testmazes\size{0}\logfile_maze{1}.xlsx".format(n,n))

            print('folder and logfile created for size{0}'.format(n))'''